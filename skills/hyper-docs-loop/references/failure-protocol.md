...
- **Teammate role-name:** `documenter`. Reused from `agents/documenter.md` (the same agent normally dispatched by `hyper-docs-sync` for UPDATE/CREATE mode); the loop's spawn-prompt contract overrides the dispatch context to "persistent teammate, structured-findings mode". This is also `teammate_name = "documenter"` — the PRIMARY handle tried first on every first send, per the §A send-resolution procedure in `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md`.
...
- **Send-resolution:** all lead→documenter `SendMessage` `to:` addresses (correctives, teardown) are resolved via the §A send-resolution procedure in `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md`. The documenter spawn is non-soliciting; the FIRST lead→documenter `SendMessage` is EITHER the Step 7 fix-round findings send (when blocking findings exist) OR the Step 8 teardown (on a clean run where no fix round is needed) — both addressed to bare `teammate_name` on the live-mailbox main path. By the time any §1 or §3 corrective fires, the first send has already succeeded; correctives still phrase the send via the §A procedure, not as a hardcoded handle.
[DEGRADE] On a degraded host the FIRST lead→documenter send triggers §A-DEGRADE D1's bare-name→`teammate_id` fallback and caches the winning handle into `resolved_handle`; later degraded sends go to `resolved_handle` directly. See §A-DEGRADE D1 in `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md`.
...
[DEGRADE] On a degraded run the documenter's reply arrives as the documenter's task-completion result (read per §A-DEGRADE D2 in `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md`), NOT as unsolicited mailbox traffic — a D2-read reply is never routed through the §B idle-correction.
