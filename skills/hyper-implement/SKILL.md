...
Read the resolved plan's opening lines. If the file begins with a YAML frontmatter block containing `tier: epic`, it is an epic **roadmap**, not an executable task plan (`hyper-plan` emits these for oversized tasks, under `.hyperclaude/epics/`). **STOP** before any branch or git work and tell the user, **substituting the resolved roadmap path** into the command: "This is an epic roadmap (`tier: epic`), not an executable task plan. Expand a milestone into a detailed plan first — `/hyperclaude:hyper-plan <this roadmap's path> milestone <K>` (K is one of the `## Milestone N:` entries in this roadmap; passing the path targets THIS epic — omitting it auto-picks the *newest* roadmap instead, which may be a different file) — then run hyper-implement on that detailed plan." Plans without `tier: epic` frontmatter (all detailed plans, including the auto-expanded Milestone-1 plan) proceed normally — a detailed plan may carry a hook-stamped `plugin-version` frontmatter block, but that is not `tier: epic`, so it is not a roadmap and proceeds. Roadmaps live in `.hyperclaude/epics/`, not `.hyperclaude/plans/`, so the newest-plan auto-pick in Step 1 never selects one — this guard only fires when a roadmap path is passed explicitly.
...
- **Archive the plan when implementation is complete.** "Complete" = every task block executed (committed or skipped-no-change) AND the final acceptance above passed. This is the *plan-implemented* signal — it does NOT depend on the optional code-review's findings (a code review is downstream hardening; its findings are bug-fix work on already-implemented code, not plan-implementation state) and is NOT the plan's checkbox count (audit / no-op tasks never get a `- [x]`, so a fully-executed plan can still show unchecked boxes). This applies to nested runs too: when `hyper-implement-loop` invokes this skill the plan is still archived here at implementation completion — the loop's later review/fix rounds operate on already-implemented code and never re-read the plan. Archive **only a canonical plan** — resolved path is a direct child of `.hyperclaude/plans/`; an explicit `$ARGUMENTS` path elsewhere is left untouched (moving it would be destructive and would violate the "only `plans/` is archived" contract). Use a plain `mv` (`.hyperclaude/` is gitignored, so `git mv` fails):
