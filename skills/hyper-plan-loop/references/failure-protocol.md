...
See `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md` §C — Teardown 3-step procedure + recovery. Plan-loop binds `<loop-name>` = `hyper-plan-loop` for the named-loop-report on terminal `TeamDelete` failure.
