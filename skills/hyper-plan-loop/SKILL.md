...
Write the planner's returned plan body verbatim with the Write tool to the resolved plan path from Step 1.

**In-place rule:** every later revision overwrites THIS SAME path; never write `-v2.md` (or any other) sibling files — the bridge's `--resume` keys on the plan path, and a new path breaks resume continuity.

If this `Write` fails → Step 8 teardown, then STOP (the planner teammate is already live).
...
SendMessage({
  ...
  message: "<verbatim Blocker/Major findings + relevant ### Verdict text when it explains the required direction; instruct the planner to return ONLY the complete revised plan body, same format contract>"
})
```

Do NOT re-send the task or research — the planner still holds that context.

**Format gate on the reply:** if the planner's response lacks `## Task N:` sections (or is otherwise not a plan body), send once more:

```
SendMessage({
  to: "planner",
  summary: "Reformat: return full plan body only",
  message: "<ask for a correctly-formatted full plan body, same format contract>"
})
```

If the second reply is still malformed → Step 8 teardown, then STOP with a named-loop report (**"hyper-plan-loop planner format, iter N"**).

On a valid reply, overwrite the same plan path with the Write tool (if it fails → Step 8 teardown, then STOP). Increment the iteration counter and re-invoke the bridge via the Bash tool with `timeout: 600000`:
...
**Teardown is MANDATORY on EVERY exit path once the Step 3 teammate spawn has succeeded** — loop success, cap reached, every bridge/format/Write/Read failure routed here from Steps 4–7, and any other unexpected tool error while the planner teammate is live. Run teardown FIRST, then report/STOP — never before.
