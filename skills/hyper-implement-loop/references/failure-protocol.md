...
Operational backstops for `hyper-implement-loop`. The shared cross-loop protocol (team contract, unsolicited-message protocol skeleton, teardown procedure, abstract request-id state machine §E, shared anti-patterns) lives in `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md`. This file is the implement-loop's binding layer: it names the teammate role (`fixer`), the reply-token shape (`request-id: <id>` prefix on the structured findings schema), the structured-schema accept rule, the semantic finding-map post-acceptance validation, and the implement-loop-specific anti-patterns. SKILL.md Step 0 Reads BOTH files.
...
- **Send-resolution:** all lead→fixer `SendMessage` `to:` addresses (correctives, teardown) are resolved via the §A send-resolution procedure in `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md`. The fixer spawn is non-soliciting; the FIRST lead→fixer `SendMessage` is EITHER the Step 7 fix-round findings send (when blocking findings exist) OR the Step 8 teardown (on a clean run where no fix round is needed) — both addressed to bare `teammate_name` on the live-mailbox main path. By the time any §1 or §3 corrective fires, the first send has already succeeded; correctives still phrase the send via the §A procedure, not as a hardcoded handle.
[DEGRADE] On a degraded host the FIRST lead→fixer send triggers §A-DEGRADE D1's bare-name→`teammate_id` fallback and caches the winning handle into `resolved_handle`; later degraded sends go to `resolved_handle` directly. See §A-DEGRADE D1 in `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md`.
...
On any gate failure, mint a new id per `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md` §E's mint protocol, then send ONE corrective message that carries the new id, addressed via the §A send-resolution procedure (the first send has already succeeded by the time a §1 gate failure fires — the §A procedure addresses to bare `teammate_name`):
...
The redo reply must echo `request-id: <new id>`. If the next reply still fails the anchored gate → Step 8 teardown, then STOP (**"hyper-implement-loop reply-contract failure"**).

## §2 — Lead-side unsolicited-message protocol

See `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md` §B — Unsolicited-message protocol skeleton. The implement-loop binds `<loop-bound reply-token>` = the `request-id: <id>` first line + structured findings schema body and `<loop-name>` = `hyper-implement-loop`. The full interplay-with-§E paragraph (Phase 1 / Phase 2 routing for id-bearing traffic vs. non-id-bearing unsolicited traffic) is in shared §B; do not duplicate it here.
[DEGRADE] On a degraded run the fixer's reply arrives as the fixer's task-completion result (read per §A-DEGRADE D2 in `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md`), NOT as unsolicited mailbox traffic — a D2-read reply is never routed through the §B idle-correction.
...
## §5 — Anti-patterns (implement-loop specific)
...
See `${CLAUDE_PLUGIN_ROOT}/references/loop-protocol.md` §E. Implement-loop's bindings for the binding holes (reply-token shape, accept rule, post-acceptance validation stage) are declared in the "Binding declarations" section at the top of this file.
