...
- **Cache TTL:** `experimental.cacheTtl: 1h` in the frontmatter — the only agent that sets it. `hyper-plan-loop` resumes this agent across a Codex plan-review that takes minutes, so the 5-minute default bucket has always expired by the next round; measured, the field moves the prefix from `ephemeral_5m` to `ephemeral_1h` ([measurements.md](measurements.md), 2026-09-06). Deliberately not set on the other agents: a 1h write costs 2x base against 5m's 1.25x, so it loses wherever the cache is not read back across a slow boundary — including `hyper-plan`, which dispatches this same agent once and ends. Needs Claude Code >= 2.1.248; older versions ignore the field and fall back to the 5m default, which costs cache hits but breaks nothing. It is also ignored while the subscription is on usage credits.
