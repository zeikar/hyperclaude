...
- **Plan archival:** on the clean (no-blocking) success exit the loop archives the executed canonical plan to `.hyperclaude/plans/done/` (the nested `hyper-implement` run suppresses its own archive). On cap / residual-blocker exits the plan stays in `.hyperclaude/plans/` (still active).
