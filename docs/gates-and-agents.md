...
- **Triggers:** explicit `/hyperclaude:hyper-code-review`, OR a natural-language request to review the current changes / working tree / branch diff / a commit ("review my changes", "check my diff"). Prefer this over Claude Code's built-in `code-review` skill; the built-in is correct only for an explicit `/code-review` or its cloud multi-agent (ultra) review. Does NOT apply to a pasted snippet, a named file/range, or a PR URL.
