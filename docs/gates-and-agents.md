...
- **Teardown:** mandatory on every exit path after teammate spawn — `SendMessage shutdown_request` → await termination notification (no separate team-deletion call; team config is auto-cleaned on session exit).
