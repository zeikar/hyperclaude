...
- **Writes:** detailed task plan → `.hyperclaude/plans/<timestamp>-<slug>.md` (plain markdown, no frontmatter, `## Task N:` sections that `/hyperclaude:hyper-implement` consumes directly). Oversized task → a `tier: epic` roadmap under `.hyperclaude/epics/<timestamp>-<slug>.md` PLUS a runnable `.hyperclaude/plans/<timestamp>-<slug>.md` detailed plan for Milestone 1 (canonical slug, so the `research → plan → plan-review` trace holds). The roadmap carries the only `tier:` frontmatter; detailed plans stay frontmatter-less. `/hyperclaude:hyper-implement` refuses a `tier: epic` file, and the roadmap living outside `.hyperclaude/plans/` keeps it off the newest-plan auto-pick entirely. Later milestones are expanded by re-running `/hyperclaude:hyper-plan <Milestone K scope>` — currently a standalone plan with its own slug and no automatic epic-dependency linkage (epic-aware re-expansion is a deferred follow-up).
