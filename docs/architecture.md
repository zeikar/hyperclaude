...
│   ├── hyper-plan-loop/         gate — autonomous plan-revise loop (persistent claude -p planner session)
...
├── references/                  plugin-wide reference content not owned by any single skill. loop-protocol.md — Step-0 base for hyper-implement-loop and hyper-docs-loop (NOT hyper-plan-loop, whose planner is a bridge session); carries the shared spawn contract (no `name:`, address by returned agentId), the reply transport (the task notification's `<result>`), the corrective/transport-failure rules, and the cross-loop anti-patterns. review-brief.md — shared `--review-brief` composition rules (source/omission/bound/shell-safety), pointed at by hyper-plan-review, hyper-code-review, hyper-plan-loop, and hyper-implement-loop. bridge-review-calls.md — shared bridge stdout JSON envelope + `--resume` semantics + invocation mode (the standalone gates and `hyper-plan-loop` background the spawn; `hyper-implement-loop` and `hyper-docs-loop` do not), pointed at by hyper-plan-review, hyper-code-review, hyper-plan-loop, hyper-implement-loop, hyper-docs-review, and hyper-docs-loop
...
`scripts/planner-bridge.mjs` is the plugin's second spawning entry point, and the only one that spawns `claude` rather than `codex`. It runs the `planner` agent as a persistent `claude -p` session for `hyper-plan-loop`:

```bash
node scripts/planner-bridge.mjs --workflow <key> --start --prompt-file <path>   # start
node scripts/planner-bridge.mjs --workflow <key> --prompt-file <path>           # continue
node scripts/planner-bridge.mjs --workflow <key> --end                          # discard
```

The agent definition is NOT duplicated — the child is launched with `--agent hyperclaude:planner`, so `agents/planner.md`'s system prompt, `model:`, and tool allowlist all still apply, and so does the project's `CLAUDE.md`.

**Why a separate process.** A native subagent writes its prompt cache into the 5-minute TTL bucket; a `claude -p` conversation writes the 1-hour bucket. Every plan-loop round is separated by a Codex review that takes minutes, so the subagent cache had always expired by the next round. This is scoped to `hyper-plan-loop` deliberately — see [decisions.md](decisions.md) for why `hyper-plan` keeps the native subagent.

**Permission mode.** The child is launched with `--permission-mode auto`. The planner writes the plan file itself (caller-directed write-file mode) and inspects the repo with `Bash`, and a headless session has nobody to approve either. Measured against the real CLI in a scratch repo, same prompt each time: `default` blocked every `Write` to the plan path and produced no file; `acceptEdits` wrote the plan but denied all three `Bash` calls, so the plan was composed without looking at the tree; `auto` wrote the plan with zero denials. `auto` is the narrowest mode that satisfies the whole contract, and it still honours configured deny rules. Note that passing a mode explicitly also pins it for a host whose own settings are more permissive.

**Session lifecycle.** One session per planning workflow, keyed by the plan file's stem (`<YYYYMMDD-HHMM>-<slug>`) — deliberately NOT the bare slug, which is derived from the first few task words and which two different workflows can mint identically; the timestamp prefix is what makes the key unique. The id lives in `.hyperclaude/planner-sessions/<key>.id`: minted by `--start` with an exclusive create, reused by every later call for that key, discarded by `--end` at every terminal step including a STOP — except after a failed `--start`, which owns no session to end and whose key may belong to another live run. **Start and continue are explicit, never inferred from whether the mapping exists.** The plan path is chosen by a check-then-use, so two runs can resolve the same stem; the exclusive create makes the claim atomic and the loser FAILS with `spawned:false` instead of joining the winner, since two tasks sharing one conversation would let either one's `--end` close the other. A continue with no mapping fails too, rather than running a findings-only revise in a brand-new planner, and an unreadable or malformed mapping is an error before `claude` starts. Cleanup verifies the file still holds its own session id before removing it: a failed `--start` releases what it minted, a failed continue leaves the established session alone, and a cancellation cleans up either way because nothing is left to run the terminal `--end`.

**`--agent` is re-passed on every call, resumes included.** Dropping it on a resume changes the system prompt and throws the cached prefix away — measured `cache_read` 17,343 with it against 4,132 without, on the same session.

**Recursion guard.** The child is spawned with `HYPERCLAUDE_ROLE=planner`, and `hooks/session-start-reminder.mjs` exits immediately when that variable is set, so the child never receives the workflow router and cannot re-enter the cycle it is a step of. Nothing else the planner needs comes from that hook.

**Contract.** One JSON object on stdout: `{"ok":true,"workflow","sessionId","resumed","spawned","body"}` or `{"ok":false,"error","spawned",…}`. Exit 2 for argv errors, 1 for a failed run; a top-level catch keeps the single-envelope contract even on an unexpected throw. **`spawned` is what a caller keys its failure handling on**: `false` means `claude` never started and nothing can have been written, `true` means the planner ran and may already have modified the plan. Like the Codex bridge it imposes no wall-clock deadline; `SIGINT`/`SIGTERM` are forwarded to the child and then **re-raised on the bridge itself**, so a cancellation surfaces as a signal death rather than an ordinary exit-1 failure. The child is spawned with an argv array carrying flags only; the prompt is read from `--prompt-file` and **piped over stdin**, as the Codex bridge already does for its own spawns. Keeping it out of argv is load-bearing twice over: Step 2 inlines whole research artifacts, so the prompt is unbounded and a positional could exceed the platform argv limit, and a revise prompt opens with verbatim Codex findings (`- **Major** …`), which the CLI would parse as a flag.
