...
├── references/                  plugin-wide reference content not owned by any single skill (currently: loop-protocol.md — Step-0 base for hyper-plan-loop, hyper-implement-loop, and hyper-docs-loop; its §A-DEGRADE section is the removable degraded-host override — see decisions.md 2026-06-21)
