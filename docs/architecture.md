...
- `--effort <low|medium|high|xhigh|max|ultra>` has no default. When omitted, Codex inherits the user's `~/.codex/config.toml` reasoning-effort setting. Valid values are exactly those six — the levels the Codex model catalog offers, not all of them on every model; `none` and `minimal` are rejected (in no catalog entry). The bridge passes it as `-c model_reasoning_effort=<value>`. Available on all four modes; v1 is bridge-only.
