...
Smoke also carries static content assertions over the three `*-loop` skills. Spawn transport (`hyper-implement-loop` and `hyper-docs-loop` only — `hyper-plan-loop`'s planner is a `claude -p` bridge session, asserted separately by its own planner-bridge invariants): each loop spawns its agent by `subagent_type` with NO `name:` field — paired PRESENT/ABSENT checks, so losing the spawn or reintroducing `name:` both fail — and `references/loop-protocol.md` must name the task-notification `<result>` reply transport. Binding holes: the shared protocol stays free of any loop-specific reply token, while each loop's own `SKILL.md` + `references/failure-protocol.md` pair MUST carry it (`WROTE:` for plan-loop, `files-changed:` for implement-loop and docs-loop), and the two `Agent`-transport loops must reference the shared protocol at Step 0 (`hyper-plan-loop` instead carries planner-bridge invariants: it invokes `scripts/planner-bridge.mjs`, keys its session on `--workflow` rather than the collidable slug, ends it with `--end`, and holds no `agent_id`/`SendMessage` reference). Deletion locks: no loop file may reintroduce the teammate-era machinery tokens (the experimental env gate, `[DEGRADE]`, `shutdown_request`, the request-id counters and reply prefix, `teammate_name`) or a `§` cross-reference into the deleted protocol sections. Plus plan-loop wording locks: the `10 total reviews` cap phrasing is present and the legacy Minor-gating fragments are absent.
...
Note: Claude Code `>= 2.1.232` is the known-good floor for the background-agent transport used by `hyper-implement-loop` (the autonomous implement-hardening loop skill), `hyper-docs-loop` (the autonomous docs-hardening loop skill), and `hyper-auto` (which chains the plan + implement inner loops) — so it only matters when dogfooding those three. `/hyperclaude:hyper-setup` reports it; nothing gates on it, and all other skills work regardless.
