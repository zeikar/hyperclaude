...
```bash
node --test tests/*.mjs            # unit tests for the bridge, setup-doctor, and hyper-memory extraction (includes tests/setup-doctor.test.mjs, tests/memory-extract.test.mjs)
...
The unit tests cover argument parsing (including the `--background` flag: acceptance, rejection of invalid combinations, mutual-exclusion with `--resume`, fresh-spawn `### Change context` injection, and fence-collision guard), slug derivation, frontmatter rendering, file-collision handling, and per-mode invocation planning; `tests/setup-doctor.test.mjs` covers the prerequisite probe (Node, codex, git, agent-teams checks); `tests/memory-extract.test.mjs` covers `scripts/memory/extract.mjs` with hermetic `mkdtemp` fixtures — corpus enumeration, identity derivation, span/verdict helpers, compound-key stability, candidate rendering, and cross-dir (`candidates/` + `promoted/`) idempotency. The smoke script:
...
- `code-review.md` — fresh `code-review` prompt. Substitutes `{{TARGET_INSTRUCTION}}` (the per-target git-command block) and `{{REVIEW_BACKGROUND}}` (optional caller-supplied context injected under a `### Change context` heading when `--background` is passed; empty string otherwise). Codex runs those git commands itself under the read-only sandbox to collect the diff — there is no native diff capture; the bridge spawns `codex --search exec --sandbox read-only -` with this rendered prompt, exactly like the other fresh modes.
...
The version is recorded in research / plan-review / docs-review / code-review output frontmatter so consumers can detect prompt drift. Plan-review emits `template-version: 2` (anti-over-engineering rubric reframe in v0.16.0); code-review emits `template-version: 3` (v2 added the over-engineering lens; v3 added the `{{REVIEW_BACKGROUND}}` slot for the `--background` channel); research and docs-review are at `1`. Only `code-review`'s `--resume` enforces a version match: a prior code-review artifact whose `template-version` doesn't match the current `templates/codex/code-review.md` frontmatter is not resumable (e.g. a legacy artifact from the old native `codex exec review` path). With `--resume auto` the bridge silently falls back to a fresh run (stderr note); with an explicit `--resume <legacy-path>` it exits non-zero with `resume rejected`. The resumed variant `code-review-resumed.md` is itself unversioned — bump it freely; the gate keys on the prior artifact's `template-version`, not the resumed template's.
