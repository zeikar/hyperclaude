...
The version is recorded in research / plan-review / docs-review / code-review output frontmatter so consumers can detect prompt drift. Plan-review emits `template-version: 2` (anti-over-engineering rubric reframe in v0.16.0); code-review emits `template-version: 3` (v2 added the over-engineering lens; v3 added the `{{REVIEW_BACKGROUND}}` slot for the `--background` channel); docs-review is at `2` (v2 added the redundancy scope); research is at `1`. Every resumable mode's `--resume` (`plan-review` / `docs-review` / `code-review`; research has no resume path) enforces a version match: a prior artifact whose `template-version` doesn't match the current fresh template's frontmatter is not resumable (e.g. a legacy code-review artifact from the old native `codex exec review` path, or an artifact from before a prompt change). With `--resume auto` the bridge silently falls back to a fresh run (stderr note); with an explicit `--resume <old-path>` it exits non-zero with `resume rejected`. The resumed variants (`*-resumed.md`) are themselves unversioned — bump them freely; the gate keys on the prior artifact's `template-version`, not the resumed template's.
