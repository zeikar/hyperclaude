...
When changing a template, bump `template-version` in lock-step. Research and plan-review share `renderFrontmatter()` but their versions are split per-mode at the call site — the live shape is `templateVersion: args.mode === 'plan-review' ? 2 : 1` in [scripts/codex-bridge.mjs](../scripts/codex-bridge.mjs). Bump the branch for whichever mode's template you changed; don't sweep both together unless both prompts actually changed. Docs-review carries its own hardcoded `template-version: 1` line in `renderDocsReviewFrontmatter()`.

`code-review` has **three** lock-step points: the prompt body `templates/codex/code-review.md`, the hardcoded `template-version: 1` line in `renderCodeReviewFrontmatter()`, and the `CODE_REVIEW_TEMPLATE_VERSION` constant in [scripts/codex/resume.mjs](../scripts/codex/resume.mjs) (the resume gate). Bump all three together when changing the code-review prompt.

The version is recorded in research / plan-review / docs-review / code-review output frontmatter so consumers can detect prompt drift. Plan-review emits `template-version: 2` as of v0.16.0 (the anti-over-engineering rubric reframe); research stays at `1`. Only `code-review`'s `--resume` enforces a version match: a prior code-review artifact lacking a matching `template-version` (e.g. a legacy artifact from the old native `codex exec review` path) is not resumable. With `--resume auto` the bridge silently falls back to a fresh run (stderr note); with an explicit `--resume <legacy-path>` it exits non-zero with `resume rejected`. The resumed variant `code-review-resumed.md` is itself unversioned in frontmatter — bump it freely; the gate keys on the prior artifact's `template-version`, not the resumed template's.
