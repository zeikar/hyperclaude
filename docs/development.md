...
Output goes to mode-specific subdirectories of `.hyperclaude/` by default — `.hyperclaude/research/`, `.hyperclaude/plan-reviews/`, `.hyperclaude/code-reviews/`, `.hyperclaude/docs-reviews/`. Override with `--out`. `--resume` is supported for `plan-review`, `docs-review`, and `code-review`; not `research`. See [architecture.md](architecture.md#cli-surface) for the full flag reference.
...
- When `codex` is on PATH, runs six Codex surface probes. Three check spawn argv by exit status: `codex exec resume --help`, `codex exec resume --help -c sandbox_mode=read-only` (verifies the `-c sandbox_mode=read-only` config key is accepted on the resume path), and `codex --search exec --help` (verifies the global `--search` flag is accepted before the subcommand — required since every bridge spawn now includes `--search`; this probe also covers the fresh code-review surface, which is `codex --search exec --sandbox read-only -` like the other fresh modes). Three cover effective-model resolution: `codex doctor --json`'s `checks["config.load"].details.model` shape, the `codex debug models` catalog shape (`slug` / `priority` / `visibility`, at least one `list` entry), and `getCodexEffectiveModel()` itself returning a non-empty, non-placeholder string. The last three judge codex's stdout CONTENT, not codex's exit status — `getCodexEffectiveModel()` deliberately ignores doctor's exit code, so gating on it here would false-negative on an offline or expired-auth machine. Each probe failure prints an upgrade hint or names the recording that would silently stop.
- When `claude` is on PATH, runs `claude plugin validate .` to catch manifest drift.

After the automated checks it prints a **post-release dogfooding checklist** for running each slash command end-to-end inside Claude Code — those steps are not automated, and per the repo's acceptance policy they are flows to exercise after tagging, not a pre-tag gate. `/hyperclaude:hyper-setup` is the last step in the printed checklist (step 11).

Smoke also carries static content assertions over the three `*-loop` skills. Spawn transport: each loop spawns its agent by `subagent_type` with NO `name:` field — paired PRESENT/ABSENT checks, so losing the spawn or reintroducing `name:` both fail — and `references/loop-protocol.md` must name the task-notification `<result>` reply transport. Binding holes: the shared protocol stays free of any loop-specific reply token, while each loop's own `SKILL.md` + `references/failure-protocol.md` pair MUST carry it (`WROTE:` for plan-loop, `files-changed:` for implement-loop and docs-loop), and every loop SKILL must reference the shared protocol at Step 0. Deletion locks: no loop file may reintroduce the teammate-era machinery tokens (the experimental env gate, `[DEGRADE]`, `shutdown_request`, the request-id counters and reply prefix, `teammate_name`) or a `§` cross-reference into the deleted protocol sections. Plus plan-loop wording locks: the `10 total reviews` cap phrasing is present and the legacy Minor-gating fragments are absent.
...
Note: Claude Code `>= 2.1.232` is the known-good floor for the background-agent transport used by `hyper-plan-loop` (the autonomous plan-revise loop skill), `hyper-implement-loop` (the autonomous implement-hardening loop skill), `hyper-docs-loop` (the autonomous docs-hardening loop skill), and `hyper-auto` (which chains the plan + implement inner loops) — so it only matters when dogfooding those four. `/hyperclaude:hyper-setup` reports it; nothing gates on it, and all other skills work regardless.
...
- Every codex invocation that starts a model turn goes through `runCodexExec` or `runCodexResume`. Don't add new spawn paths without re-checking the sandbox argument. The non-model probes — `codex --version`, `codex doctor --json`, `codex debug models` — are the exception: they run no turn, so no sandbox argument applies.
...
- `--model <name>` and `--effort <low|medium|high|xhigh|max|ultra>` are available on all four modes. Both have no bridge default (null = inherit `~/.codex/config.toml`). The six `--effort` values are the levels the `codex debug models` catalog offers across models; a given model may support fewer (Codex rejects those), while `none` (plan-mode only in the Codex CLI) and `minimal` (in no catalog entry) are rejected by the bridge. Selection tokens are inserted into argv after the subcommand and before `--sandbox`/`-c sandbox_mode=read-only`, preserving the sandbox invariant. The two flags record new frontmatter keys (`codex-model-requested`, `codex-effort-requested`) only when passed; they are omitted from the artifact when not used. Separately, every non-dry-run spawn resolves the model it actually runs under and records it as `codex-model-effective` — which costs a `codex doctor --json` probe (plus, on a `<…>` placeholder, a `codex debug models`) on the flagless path; passing `--model` short-circuits both probes.
