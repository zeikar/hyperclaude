...
The version is recorded in research / plan-review / docs-review / code-review output frontmatter so consumers can detect prompt drift. Plan-review emits `template-version: 2` as of v0.16.0 (the anti-over-engineering rubric reframe); the other three are at `1`. Only `code-review`'s `--resume` enforces a version match: a prior code-review artifact whose `template-version` doesn't match the current `templates/codex/code-review.md` frontmatter is not resumable (e.g. a legacy artifact from the old native `codex exec review` path). With `--resume auto` the bridge silently falls back to a fresh run (stderr note); with an explicit `--resume <legacy-path>` it exits non-zero with `resume rejected`. The resumed variant `code-review-resumed.md` is itself unversioned — bump it freely; the gate keys on the prior artifact's `template-version`, not the resumed template's.
