...
The 2026-06-20 addressing-reversal migration adds agent-teams contract assertions across all three loop SKILLs: a negative lock that `to: teammate_id` (the stale id-only form) does NOT appear in loop SKILL.md files or in `references/loop-protocol.md`; a positive lock that §A first-send-fallback logic is present (bare-name primary → `agent_id` fallback → cache resolved handle); per-loop `teammate_name` role-binding assertions; a `{ type: "shutdown_request" }` object-shape lock on teardown (plain string `message` form is rejected); and the no-wait + v2.1.178 blocks retained from the prior migration. Teardown steps must NOT assert a wait for termination notification or a `shutdown_response`. The manual acceptance checklist for `hyper-plan-loop`, `hyper-implement-loop`, and `hyper-docs-loop` includes: (a) confirm the first SendMessage in a live run tries the bare spawn name first; (b) on a host where the bare name is rejected ("No agent named X is currently addressable"), confirm the `agent_id` fallback is used and the resolved handle is cached; (c) confirm teardown sends `{ type: "shutdown_request" }` and does not block waiting for a reply; (d) on a degrade (e.g. env var unset), confirm the loop STOPs cleanly without a teardown attempt.
...
The version is recorded in research / plan-review / docs-review / code-review output frontmatter so consumers can detect prompt drift. Plan-review and code-review both emit `template-version: 2` (plan-review's anti-over-engineering rubric reframe in v0.16.0; code-review's added over-engineering lens, holding code to the same simplicity bar plan-review holds the plan to); research and docs-review are at `1`. Only `code-review`'s `--resume` enforces a version match: a prior code-review artifact whose `template-version` doesn't match the current `templates/codex/code-review.md` frontmatter is not resumable (e.g. a legacy artifact from the old native `codex exec review` path). With `--resume auto` the bridge silently falls back to a fresh run (stderr note); with an explicit `--resume <legacy-path>` it exits non-zero with `resume rejected`. The resumed variant `code-review-resumed.md` is itself unversioned — bump it freely; the gate keys on the prior artifact's `template-version`, not the resumed template's.
