...
### agent-teams loop protocol — addressing, teardown, degrade (superseded by the 2026-08-17 entry below)
...
**(b) Gate is per-field, not all-or-nothing.** Each key is written independently when ITS specific field in `turn.completed.usage` is non-null (`if (usage.input_tokens != null)` etc.), so a partial usage object yields only the keys whose fields were present. In practice codex-cli >= 0.130.0 emits all four fields together, but the contract is per-field — preserving 0 values while omitting absent ones. Keys are omitted entirely when `turn.completed.usage` was absent from the JSONL stream (typically when Codex exited before emitting `turn.completed`). A run that emitted usage but then failed (non-zero exit, a reported turn failure, etc.) WILL still record the applicable keys in its failure artifact — those tokens were genuinely spent and recording them aids cost tracking. This mirrors the existing `if (codexThreadId)` omit-when-absent pattern used for `codex-thread-id`.
...
### 2026-08-06 — plan revise rounds must re-ground; the plan stays a task list

Plan-loop round counts were measured across two dogfooding repos. Both failure modes traced to the same asymmetry: Codex re-reads the repo on every resumed round (read-only sandbox, 500–800k input tokens per round), while the planner's revise rounds read almost nothing — in one seven-round run the initial draft did 25 `Read` + 19 `Bash` calls and revise rounds 2–6 did zero of either, only `Edit`. A grounded reviewer against an ungrounded reviser produces findings that survive revision: "Prior … remains" / "still" recurred 4–9 times per round in the long series, one finding surviving four consecutive rounds. `agents/planner.md` now requires re-reading what a finding cites before editing and fixing at the source; `hyper-plan-loop`'s Step 6 message repeats it, since the loop's revise instruction previously named only the plan file.
...
### 2026-08-17 — loops drop the named teammate for a no-`name:` spawn addressed by `agentId`

Passing `name:` to the `Agent` tool made the agent a team member, which triggers anthropics/claude-code #78234 / #81746: the harness drops the plugin agent definition, so the `tools:` allowlist is lost, an ~18KB skill listing is re-attached every round, the prompt cache is invalidated, and cost grows quadratically in round count. A spawn WITHOUT `name:` keeps the definition and the cache — the agent runs as a background task whose final text arrives as the task-notification `<result>`, and `SendMessage({ to: "<agent_id>" })` reaches it on later rounds with context intact. That transport forms no team, so the machinery built around one is gone: the request-id state machine, the `§A-DEGRADE` layer, the unsolicited-message protocol, shutdown/teardown, the `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` precondition, and the dual SessionStart router template (one loop-first `session-start-reminder.md` remains, hardcoded). What survived is everything that was judgment rather than plumbing: reply validation, corrective budgets, severity gates, review caps, `--resume auto` bridge mechanics, and single-spawn context reuse. Two riders: implement-loop and docs-loop now spawn their agent **lazily**, on the first blocking-findings round only (a clean first review spawns none — plan-loop's spawn is still its first solicitation, since it writes the plan), and setup-doctor's check 5 was repurposed from the env var to *reporting* `Claude Code >= 2.1.232`, the known-good floor for this transport. That check is `conditional` severity, so a below-floor or unknown version is a WARN that never flips the aggregate `ok` — nothing gates an unsupported host, which under the pre-adoption policy is an accepted gap (`hyper-setup` is explicit-invoke and report-only by design). The teammate-era entries above stay as historical record.

**Measured and accepted: the subagent prompt cache has a 5-minute TTL.** Subagents write their cache as `ephemeral_5m` (that field carried the whole write; `ephemeral_1h` was zero), so any gap longer than five minutes re-writes the agent's accumulated context at full price. The five-minute rule classified gap-vs-miss perfectly across 8 observed round boundaries: intra-round gaps of 71–154s hit with `cache_read` 131k–217k, while both boundaries containing a Codex review missed (469s and 1393s, `cache_read=0`). The exposure is therefore review-bearing boundaries, not every round. This is INDEPENDENT of the `name:`/skill-listing mechanism above, which the no-`name:` spawn does fix.

Both available fixes were simulated over 480 local subagent transcripts and both lose, so this is closed rather than deferred. **(a) 1h TTL.** Claude Code picks the TTL from a `querySource` allowlist (`repl_main_thread*`, `sdk`, `auto_mode`, `memdir_relevance`) that subagents never match; `ENABLE_PROMPT_CACHING_1H=1` overrides it for every source. But a 1h write costs 2x base vs 5m's 1.25x, so break-even needs expiry-driven tokens to be 39% of all write tokens — the loosest classification puts them at 19%. Simulated net: **18% worse**. (Also: `isUsingOverage` forces 5m regardless, which is why cache behaviour degrades further past a usage limit.) **(b) Keepalive.** The timer objection was wrong — a backgrounded `sleep N; echo PING` gives the lead an alarm with no scheduler in skill prose (verified). It fails on value instead: a cache read refreshes the TTL at 0.1x, but capping refreshes at the optimum (3) nets only ~5% of subagent weight — roughly **1% of session weight** — while 21 of 105 boundaries still spend the budget and expire anyway, because gap length is unknowable when the refresh is scheduled. Resume therefore keeps paying one re-write per review-bearing round; the review caps (10 / 6) are what bound that cost, since re-write size grows linearly with round count. Both simulations, the cross-tab behind the transport change, and the post-release dogfood that confirmed it are in [measurements.md](measurements.md).

### 2026-09-01 — the accretion lens: prose findings get fixed at their source, not beside it

A review loop can make every round locally correct and the text globally worse: a finding lands on a sentence, the fix adds a qualifier beside it, and five correct rounds leave a paragraph nobody can read. CLAUDE.md's rule that docs edits amend existing prose rather than append to it never reached where the appending happens — subagents don't inherit CLAUDE.md and Codex never sees it, so only a lead editing by hand was ever bound by it. Fixed on both sides, weighted toward authoring: `agents/fixer.md` now revises the sentence a prose finding is about instead of adding a correct one next to it (the `documenter` has carried the same rule since the 2026-07-12 entry above; `hyper-docs-loop`'s Step 5 constraints echo now repeats it, since that echo declares itself authoritative for the dispatch and was silently dropping it). Detection follows: code-review v5 counts diff prose that qualifies an adjacent claim instead of correcting it as over-engineering, and docs-review v3 widens redundancy from literal repetition to "a reader must read both passages to get one answer". The review half is knowingly the weaker lever — on a resumed round Codex is looking at a sentence its own prior finding asked for — so it earns its keep on fresh reviews. **Non-blocking, deliberately:** both loop gates already exclude these findings, which avoids the oscillation of round N asking for precision and round N+1 deleting what it got. No length cap, for the reason 2026-08-06 rejected one for plans. Both bumps break `--resume` across the change by design (auto → fresh fallback).

### 2026-09-02 — the bridge's wall-clock timeout is removed, not raised

`--timeout` (default 600s) is gone from all four modes and nothing replaces it — no idle reaper either; the bridge waits for Codex as long as Codex takes. The reframing that settled it: **a timeout is not a cost ceiling.** A killed run's tokens are spent either way (the usage-recording entry above already records them into the failure artifact), so killing forfeits the output without refunding anything. A deadline's only legitimate job is telling "slow but working" from "wedged", and 600s cannot do that on a large codebase. Rollout-log forensics over four `timed-out=true` artifacts in a consumer repo found **4/4 were healthy runs killed mid-flight** — one truncated while printing a finished review body, one 37s short of its response — and zero wedged. An idle reaper was rejected on the same data: the two observed stalls were 424s and 604s and both recovered unaided, so any threshold low enough to catch a hang kills them too. Runaway spend is the caller's Ctrl-C (observed worst case 6.5M tokens / 563s). The flag is deleted rather than left as a no-op, so a stale call fails loudly as an unknown flag instead of quietly lying. Removing the kill also retires a bug it owned: `codex` is an npm Node wrapper that spawns the native binary with `stdio:'inherit'`, so `child.kill()` reached only the wrapper while a surviving descendant held the stdout pipe — `close` arrived late, the artifact's `generated:` overshot the deadline by up to six minutes, and the exit signature read `status=0, signal=null`, which reads like a clean exit. Numbers and method in [measurements.md](measurements.md).

**What this does not fix.** The three `*-loop` skills still invoke the bridge in the foreground, where the Bash tool's own 600s ceiling applies and fires before the bridge can write anything — no artifact, no diagnostics. Backgrounding them would put a round's review result on a task notification alongside the loop agent's, two async channels per round, so it is left to its own cycle; the split is stated in [references/bridge-review-calls.md](../references/bridge-review-calls.md).

### 2026-09-02 — the plan-loop planner is promoted to a persistent `claude -p` session

`hyper-plan-loop`'s planner no longer runs as an `Agent`-tool subagent. It runs as one `claude -p` conversation per planning workflow, started and resumed through `scripts/planner-bridge.mjs` and keyed by the plan-file stem. The agent definition is reused verbatim via `--agent hyperclaude:planner`, so nothing about the planner's prompt, model, or tools is duplicated in code.

The reason is the cache bucket. Measured on this host: a `claude -p` turn writes `ephemeral_1h_input_tokens` 17,334 with `ephemeral_5m` at 0, while a native subagent writes the 5-minute bucket (the 2026-08-17 entry above). Every plan-loop round is separated by a Codex plan-review that takes minutes — the two review boundaries measured there were 469s and 1393s, both `cache_read=0` — so the subagent's cache had *always* expired by the time the planner was asked to revise. The 1-hour bucket survives that gap.

**Scoped to the loop on purpose: `hyper-plan` keeps the native subagent.** The 2026-08-17 finding that a 1-hour TTL is net negative still holds wherever the cache is not read back: a 1h write costs 2x base against 5m's 1.25x, so a single-shot dispatch that never resumes pays a 60% premium for nothing. `hyper-plan` dispatches the planner once and ends; even its epic path's two dispatches are back-to-back with no review between them, so a 5-minute bucket would not have expired anyway. The general conclusion and this exception are the same rule applied to different boundary profiles — the 2026-08-17 entry's own caveat, that the exposure is review-bearing boundaries rather than every round, is what makes the plan-loop planner the one place this wins.

Four implementation points that are not obvious. **The child needs `--permission-mode auto`.** The planner writes its own plan file and inspects the repo with Bash, and a headless session has nobody to approve either; measured, `default` blocked every write and produced no plan, `acceptEdits` wrote the plan but denied every Bash call, and `auto` did both with zero denials. **`--agent` is re-passed on every resume**, not just the first call: dropping it changes the system prompt and discards the cached prefix (measured `cache_read` 17,343 with it against 4,132 without, same session). **The session id is owned by the bridge**, in `.hyperclaude/planner-sessions/<key>.id`, not carried in lead run-state across turns — a loop driven by skill prose should not be asked to hold a UUID across a dozen turns, and keying it makes "one session per workflow" structural rather than a rule the lead has to follow. The key is the plan file's stem, not the bare slug: slugs come from the first few task words, so two unrelated workflows collide often enough that keying on one would resume the wrong conversation. **Recursion is blocked by `HYPERCLAUDE_ROLE=planner`** in the child env, which makes `hooks/session-start-reminder.mjs` exit before injecting the workflow router; CLAUDE.md, project instructions and the agent definition are unaffected because none of them come from that hook.

Consequence for the loop family: `hyper-plan-loop` no longer uses `references/loop-protocol.md` at all — it has no `name:`-less spawn, no `agentId`, and no task-notification reply, so its whole binding is its local `failure-protocol.md`. `hyper-implement-loop` and `hyper-docs-loop` are untouched and still use that transport.
