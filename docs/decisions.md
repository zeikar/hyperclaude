...
### 2026-06-20 — id-only teammate addressing, no-wait teardown, degrade-path branch (**Superseded** — addressing portion reversed; see "addressing contract reversed" entry below)

> **Addressing note:** the id-only routing decision in (a) below was reversed by the 2026-06-20 "addressing contract reversed: bare-name primary + agent_id fallback (host-portable)" entry. The no-wait teardown (b), degrade-path branch (d), and autonomous cleanup (e) remain current; the teardown shape was updated to the `{ type: "shutdown_request" }` object form in that same reversal entry.
...
### 2026-06-20 — addressing contract reversed: bare-name primary + agent_id fallback (host-portable)
