...
**Known limitation, accepted for now: the subagent prompt cache has a 5-minute TTL.** Subagents write their cache as `ephemeral_5m` (that field carried the whole write; `ephemeral_1h` was zero), so any gap longer than five minutes re-writes the agent's accumulated context at full price. The five-minute rule classified gap-vs-miss perfectly across 8 observed round boundaries: intra-round gaps of 71–154s hit with `cache_read` 131k–217k, while both boundaries containing a Codex review missed (469s and 1393s, `cache_read=0`). The exposure is therefore review-bearing boundaries, not every round. A keepalive was designed and rejected this cycle: any version that actually works needs periodic refreshes — a scheduler, exactly the machinery class this change deletes — so it is deferred to a future plan. This mechanism is INDEPENDENT of the `name:`/skill-listing mechanism above; the no-`name:` spawn does not address it.
