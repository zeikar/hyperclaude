...
`hyper-implement-loop` and `hyper-docs-loop` keep it foreground: their round state machine has not been re-checked against a non-blocking review turn. That leaves a ceiling worth knowing: the bridge itself imposes no deadline, but a foreground Bash call is still capped at 600s by the harness, and a loop review that exceeds it is killed before the bridge writes anything — no artifact, no diagnostics. `hyper-plan-loop` backgrounds its review turns despite having an agent like theirs, because its boundaries were measured at 469s and 1393s and a killed review costs the whole round; those two loops carry the same ceiling unmeasured. The backgrounded gates and `hyper-research` have no such ceiling.
