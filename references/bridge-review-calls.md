...
`hyper-implement-loop` and `hyper-docs-loop` keep it foreground: their round state machine sequences review turns against a live `Agent`-tool agent and has not been re-checked against a non-blocking review turn. That leaves a ceiling worth knowing: the bridge imposes no deadline, but a foreground Bash call is capped at 600s by the harness, and a review that exceeds it is killed before the bridge writes anything — no artifact, no diagnostics. `hyper-plan-loop` is no longer in that group: it has no agent (its planner is a bridge session too), so it backgrounds its planner turns — measured, a planner turn does outrun 600s on a real repo.
