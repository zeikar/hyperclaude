...
- **Which sends can be the first send:** the first send is NOT always a findings/revise round. On a clean run with no blocking findings the FIRST lead→teammate send is the **teardown `shutdown_request`** (§C — R1 only, id-exempt); in plan-loop a malformed initial `WROTE:` makes the **initial §1 corrective** the first lead→planner send (id-bearing — R1 + R2). Both invoke R1's same handle resolution.
