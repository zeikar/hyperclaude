...
This file is the shared cross-loop protocol reference, loaded at Step 0 by `hyper-plan-loop`, `hyper-implement-loop`, and `hyper-docs-loop`. Sections: §A agent-teams tool contract, §B unsolicited-message protocol, §C teardown, §D anti-patterns, §E request-id state machine, §F shared loop skeleton (the per-loop binding holes each SKILL points at). [DEGRADE] A removable §A-DEGRADE override section also sits between §A and §B. Each consuming loop's local `failure-protocol.md` is the binding layer that names the teammate role, the reply-token shape, the anchored-reply acceptance rule, and the post-MESSAGE-ACCEPTED validation stage. Consult the loop's local `failure-protocol.md` for those values.
...
**§A send-resolution procedure (every lead→teammate send invokes this).** The `<teammate-name>` placeholder in §B/§C is an abstract alias for the teammate; this procedure decides the concrete `to:` handle. It has TWO RESPONSIBILITIES (R1 handle-resolution, R2 id/`solicit_sent_at` state), NOT two sequential stages — the authoritative ordering for an id-bearing send is the "Operational order" bullet below.
...
- **[DEGRADE]** if the FIRST lead→teammate bare-name send FAILS (a degraded host whose live mailbox cannot route the bare name — distinct from line 14's later unexpected-tool-error), OR the teammate replies via its task-completion result instead of the mailbox → apply §A-DEGRADE instead of this main path.
...
- **[DEGRADE]** Rationale (degrade): degraded-host addressing and driving live in §A-DEGRADE as a cleanly-removable override (see §A-DEGRADE top).
...
[DEGRADE] The detection conditions (1)/(2)/(3) AND the dual-handle machinery (`teammate_id`, `resolved_handle`) live HERE — degrade-only, no meaning on the main path (see §A-DEGRADE top).
[DEGRADE] Detection is the bare-name send itself: trying §A R1 and having it FAIL, OR a teammate that replies via its task-completion result, IS the degrade signal — there is no separate probe. A spawn that returns no `agent_id` is NOT a degrade signal at spawn time; it only matters as "no fallback available" AFTER a bare-name send has actually failed (D1 condition (1)).

[DEGRADE] **D0 — spawn-time agent_id capture (degrade-only, opportunistic).** At the `Agent` teammate spawn, OPPORTUNISTICALLY capture the returned `agent_id` (if present) VERBATIM/OPAQUELY into run-state `teammate_id` — never parse the `@`/suffix; the format is not a documented contract. If no `agent_id` is returned, leave `teammate_id = null` — this is NOT a degrade signal and NOT a STOP at spawn time. Initialize `resolved_handle = null`. These two fields exist ONLY to support D1's fallback addressing; on the live-mailbox main path they are unused.

[DEGRADE] **D1 — agent_id addressing (POST-SPAWN solicitations only — M4).** On a degraded host a post-spawn bare-name `SendMessage` fails, so retry the SAME message exactly ONCE with `to: teammate_id` (the D0-captured handle; never parsed). Cache the handle that won into `resolved_handle` and address every later degraded send `to: resolved_handle` directly (single attempt). Genuine-STOP conditions live HERE: **condition (1)** a bare-name send FAILED and `teammate_id` was not captured at spawn (D0 captured nothing) → STOP-with-diagnostic (no fallback handle available — bare-name failed, no agent_id to retry with); **condition (3)** the first lead→teammate send fails on BOTH bare `teammate_name` AND `teammate_id` → STOP-with-diagnostic (R1+fallback exhausted). Both conditions are reached ONLY after a bare-name send has actually failed — never at spawn time. The per-loop STOP-and-preserve behavior (what state to surface, whether to preserve work) is bound in each loop's spawn step. **(m6)** for an id-bearing send delivered via `teammate_id` (the bare-name primary failed), KEEP the same request id + message payload but RECAPTURE `solicit_sent_at` (Bash `date -u +%FT%TZ`) immediately BEFORE that actually-delivered fallback `SendMessage` — §E's "capture immediately before the delivered SendMessage" invariant binds to the send that actually went out; teardown never touches `solicit_sent_at`.

[DEGRADE] **D2 — notification-reply deterministic DRIVING (re-scoped condition (2)).** On a degraded host the teammate's reply arrives as its task-completion result, NOT a mailbox `SendMessage`. The lead READS the reply text from that task result and applies the SAME verification it would on the mailbox path — the §E id-classification + the loop-bound anchored-reply gate + the loop-bound post-acceptance validation stage — identical verification and state machine, only the read source differs — then CONTINUES the loop. **Two reply-read cases (M4):** (i) **spawn-result reply** — the FIRST reply (plan-loop's initial loop-bound reply-token, because plan-loop's spawn IS a solicitation) is read from the `Agent` SPAWN task result; NO D1 send precedes it. (ii) **post-spawn-solicitation reply** — every later reply is read from the task result of a D1 (`teammate_id`) `SendMessage`. (Implement-loop and docs-loop spawns are non-soliciting, so their first reply is ALWAYS case (ii).) The deterministic sequence ENDS at "validate reply → continue the loop" — teardown is NOT a step in this sequence; it is reached only via the normal exit/teardown path (D3 / §C).

[DEGRADE] **D3 — no-wait degrade teardown (must handle `resolved_handle == null` AND `teammate_id == null`).** On the EXIT path of a degraded run, send the `{ type: "shutdown_request" }` object best-effort ONCE, then drop without waiting for any confirmation. Resolve the target with three cases in order: **(a) `resolved_handle` is set** → send to it, no-wait; **(b) `resolved_handle` is null but `teammate_id` was captured** (D2 case (i): plan-loop's initial reply came from the spawn result, no prior D1 send occurred, and Codex was clean on iteration 1 — so this teardown is the FIRST lead→teammate send) → send to `teammate_id` once, no-wait; **(c) BOTH `resolved_handle` and `teammate_id` are null** (no usable handle — genuine STOP per condition (1)/(3)) → STOP WITHOUT teardown (the §C degrade-path exception — there is no addressable teammate). This is an exit-path teardown, NOT a step in the D2 driving loop.

[DEGRADE] **Isolation note.** Removal: delete this section + every physical `[DEGRADE]` line across loop-protocol.md + the six loop files; §A/§B/§C/§E + the loop SKILL.md/failure-protocol.md then read standalone. Removal ALSO updates these EXACT doc locations (separate cleanup, not in the smoke grep): `docs/architecture.md` loop-paragraph §A send-resolution sentence + the `references/` tree comment; `docs/gates-and-agents.md` host-mirror §A send-resolution note + the degrade/teardown clauses in the hyper-plan-loop / hyper-implement-loop (incl. Teardown) / hyper-docs-loop Mechanics bullets; `docs/workflow.md` the three autonomous-alternative paragraphs' §A-DEGRADE/degraded-host/agent_id-fallback/notification-reply/no-wait-degrade-teardown clauses; `docs/development.md` the smoke-description §A-DEGRADE isolation-check sentence. `docs/decisions.md`'s 2026-06-21 entry stays as the historical record.
...
**Interplay with §E (id-first classification).** §E's two-phase state machine is the authoritative router; §B's "did I solicit anything" heuristic is only a secondary check. Division of labor: §B owns ONLY genuinely-unsolicited non-(loop-bound reply-token) traffic (nags, body echoes, `RESEND:`) — that, and only that, triggers the §B idle-correction. Everything keyed on a `reqid` (stale/duplicate loop-bound reply-tokens in either phase) routes through §E, not §B — do NOT send the §B idle-correction for a stale/duplicate reqid reply. In particular, §E Phase 2's stale-idle branch — a payload-less `idle_notification` whose `idle.timestamp < solicit_sent_at` — is NOT unsolicited (ignore silently, stay awaiting); only an idle with `idle.timestamp >= solicit_sent_at` is a true post-solicit silence and routes through the loop's local §1. This timestamp guard is the load-bearing §B↔§E hand-off; see §E Phase 2 for its full branch logic. Both the stale-reply and stale-idle cases are now identified by lead-owned state, not the old "did I solicit anything" timing heuristic.
...
- `review_iteration` — bridge re-invocation count, independent of the id counter (the id bumps on every solicitation including correctives; the iteration only on bridge re-invocation).
...
## §F — Shared loop skeleton (per-loop bindings only)

The boilerplate the three loop SKILLs share, consolidated so each SKILL can point at a named sub-block with a one-liner instead of restating it. The sub-blocks carry binding holes — `<fallback-command>`, `<teammate-role>`, `<cap-number>` — that each loop fills with its own concrete values; this section names NO concrete role or cap. Loop-specific behavior (the reply-token shape, accept rule, post-acceptance validation, cap-report framing) stays in each loop's SKILL.md / `failure-protocol.md`.

- **(F1) Agent-teams tool contract framing.** This skill uses the `Agent` and `SendMessage` tools; the exact argument shapes (spawn `subagent_type`/`name`, `SendMessage` `to`/`message`/`summary`) and the idle-notification semantics (payload-less wake signal, not the reply text) live in §A — point there rather than restating them.
- **(F2) Step 0 — protocol read.** Before spawning the `<teammate-role>` teammate, read BOTH this `loop-protocol.md` AND the sibling `failure-protocol.md`. Failure branches cite the shared §A–§E here and the local §1–§5 by number.
- **(F3) Step 2 — agent-teams probe.** Gate the loop on the experimental feature: if `[ "$CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS" = "1" ]` is not satisfied, STOP with the documented message below (each loop binds `<fallback-command>` to its own fallback command names):

  > agent teams unavailable — this skill requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`; run /hyperclaude:hyper-setup to diagnose prerequisites. Use `<fallback-command>` manually instead.
- **(F4) Step 4a — unsolicited-message handling.** Reducible to a binding pointer: the §E two-phase classification is the authoritative router — every `reqid`-bearing message routes through §E (stale/duplicate → §E, not §B); only genuinely-unsolicited non-reply-token traffic (nags, body echoes, `RESEND:`-style re-emits) is §B's domain and triggers the §B idle-correction. See §E and §B; Step 4a is just the loop's binding of those.
- **(F5) Step 8 — teardown.** Teardown procedure is defined in §C — see §C. Each loop's Step 8 keeps only its loop-specific `<cap-number>` + cap-report framing and points here for the procedure.
- **[DEGRADE] (F6) degrade conditions.** Degrade conditions (1)/(2)/(3) and the degraded-host driving/teardown detail live in §A-DEGRADE D1/D2/D3 — point there (D1=addressing, D2=notification-reply driving, D3=no-wait degraded teardown).
