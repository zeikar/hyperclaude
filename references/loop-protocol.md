...
- **(F3) Step 2 — agent-teams probe.** Gate the loop on the experimental feature: if `[ "$CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS" = "1" ]` is not satisfied, STOP with the documented message telling the user to set that env var and re-run, or fall back to `<fallback-command>` (each loop binds its own fallback command names).
